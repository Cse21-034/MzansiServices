...
const FooterNav = () => {
  ...
  const NAV: NavItem[] = [
    ...
    {
      name: session?.user ? "Dashboard" : "Log in",
      link: session?.user ? "/usersdashboard" : "/login",
      icon: UserCircleIcon,
    },
    {
      name: "List Business",
  ...
  // Hide footer nav on business, user, and admin pages to avoid duplicate navigation
  if (
    pathname &&
