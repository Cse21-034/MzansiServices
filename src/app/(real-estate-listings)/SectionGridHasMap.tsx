...
const SectionGridHasMap: FC<SectionGridHasMapProps> = () => {
  ...
  const getDefaultCenter = (): [number, number] => {
    if (DEMO_EXPERIENCES.length > 0 && DEMO_EXPERIENCES[0].map) {
      return [DEMO_EXPERIENCES[0].map.lat, DEMO_EXPERIENCES[0].map.lng];
    }
    // Default to a central location if no experiences are available
    return [0, 0]; 
  };
