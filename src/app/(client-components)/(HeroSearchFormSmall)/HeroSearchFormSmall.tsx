...
import { ServiceSearchFormFields, SearchTab } from "../type";
import BusinessSearchForm from "../(HeroSearchForm2Mobile)/(real-estate-search-form)/BusinessSearchForm";

export interface HeroSearchFormSmallProps {
  className?: string;
  defaultTab?: SearchTab;
  onTabChange?: (tab: SearchTab) => void;
  defaultFieldFocus?: keyof ServiceSearchFormFields;
}

const TABS: SearchTab[] = ["Services", "Products", "Companies"];
...
const HeroSearchFormSmall: FC<HeroSearchFormSmallProps> = ({
  ...
  const renderForm = () => {
    // For now, use BusinessSearchForm for all tabs
    // You can create separate forms for Products and Companies later
    return <BusinessSearchForm />;
  };
