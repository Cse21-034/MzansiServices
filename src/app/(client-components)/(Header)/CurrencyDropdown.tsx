      ...
      <Popover className="relative">
        {({ open, close }) => (
          <>
            ...
            >
              <Popover.Panel className="absolute z-10 w-screen max-w-[160px] px-4 mt-4 right-0 sm:px-0">
                <div className="overflow-hidden rounded-2xl shadow-lg ring-1 ring-black ring-opacity-5">
                  <div className="relative grid gap-3 bg-white dark:bg-neutral-800 p-4 max-h-60 overflow-y-auto">
                    {headerCurrency.map((item, index) => (
                      <a
                        key={index}
                        href={item.href}
                        onClick={() => close()}
                        className={`flex items-center p-2 -m-3 transition duration-150 ease-in-out rounded-lg hover:bg-gray-100 dark:hover:bg-neutral-700 focus:outline-none focus-visible:ring focus-visible:ring-[#612C30] focus-visible:ring-opacity-50 ${
                          item.active
